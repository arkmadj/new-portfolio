...
const LimitCard = () => {
  return (
    <div className="flex flex-col items-center p-5 bg-shara-background rounded-xl gap-[7px] absolute right-0 bottom-[46px] shadow-shara-2">
      <span className="text-xs font-medium text-shara-gray-4">
        Limit Available
      </span>
      <span className="text-xl font-bold text-shara-primary mb-[5px]">
        Ksh 80,000
      </span>
      ...
      {[2, 4, 8, 12].map((n) => (
        <div
          ...
          className={`flex justify-between items-center border-2 ${
            n === 2 ? "border-shara-green-2" : "border-shara-gray-2"
          } rounded-sm py-1 px-2.5`}
        >
          <div
            className={`w-3 h-3 rounded-full ring-2 ring-offset-1 ${
              n === 2
                ? "bg-shara-green-1 ring-shara-green-2"
                : "bg-white ring-shara-gray-2"
            }`}
          ></div>
          <span
            className={`${
              n === 2 ? " text-shara-green-2" : "text-shara-gray-2"
            } text-[10px]`}
          >
            {n} weeks
          </span>
